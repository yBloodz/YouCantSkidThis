...
[18248735995]                    = {Name = "Fraud Hood", Arg = "UpdateMousePos", Remote = "MainEvent"},
}
...
if game.PlaceId == 123 then
    ...
    print("[⚙️ serv.cc; Version]: Current Game Version:", tostring(CurrentVersion))
    print("[⚙️ serv.cc; Version]: Script Updated to Version", ScriptVersion)
    
    if tostring(CurrentVersion) ~= ScriptVersion then
        LocalPlayer:Kick("[⚙️ serv.cc; Version]: Serv.cc isn't updated to this new version yet. Wait for Update @ discord.gg/9sU5vNShyZ")
    else
        print("[⚙️ serv.cc; Version]: Version check passed. The script is up to date.")
    end
else
    print("[⚙️ serv.cc; System]: Checked only for Da Hood.")
end
end
...
local function injectScript()
 for i = 1, 100 do
     ...
     elseif i == 100 then
         ...
         delay(10, function()
             if not loaded then
                 game.Players.LocalPlayer:Kick("serv.cc | Failed to load correctly, please rejoin.")
             end
         end)
         loaded = true
         xuaaLoader:Destroy()
...
local function startInjection()
    addChangelogEntry(true, "serv.cc | Release", "06/30/24")
    addChangelogEntry(true, "Full Rewrite", "06/20/24")
    addChangelogEntry(true, "Streamable + Undetected >_<", "06/30/24")
    addChangelogEntry(true, "Better Security!", "06/30/24")
    addChangelogEntry(true, "Actual Bypass 4 Camlock Detection >_<", "06/30/24")
    statusText.Text = "Injecting.."
 Menu:SetVisible(false)
 injectScript()
 task.wait()
 Menu:SetVisible(true)
Menu.Notify("serv.cc has been Injected!", 5)
 Menu:Init()
end
...
if versionNumber and versionNumber < 1 then
